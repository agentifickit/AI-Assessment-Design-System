    ...
    <>
      ...
      <DocSection title="Keyboard operation" description="Everything, without exception, including the layout itself.">
        <SpecList
          ...
          { term: 'Focus visibility', detail: <>A 2px ring in <code>--brand-accent</code> with a 2px offset, at 3:1 against every surface it appears on. It follows the element's own corners. Never removed, never replaced by a colour change alone, with one exception since 2026-09-26: a document's in-page title (<code>PageTitleInput</code>) is a borderless heading, and the caret is its indicator, as in any document editor. The ring sits in the base layer so a component can move it inside its edge (menu items, tabs) or, in that one case, leave it off.</> },
